...
int main(int argc, char **argv)
{
	...
	for (size_t i = 0; cards->size(cards) && i < numbers_len; ++i) {
		val = numbers[i];
		while (cards->size(cards)) {
			card = cards->shift(cards);
			if (bingo_card_mark(card, numbers[i])) {
				winners->push(winners, card);
			} else {
				loosers->push(loosers, card);
			}
		}
		deque_s *tmp = cards;
		cards = loosers;
	...
	card = winners->peek_top(winners, 0);

	unsigned long answer = val * bingo_card_sum_unmarked(card);

	while (winners->size(cards)) {
		card = cards->shift(cards);
		free(card);
	}
	deque_free(winners);
