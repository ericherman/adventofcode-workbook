...
static void reverse_bytes_unsafe(unsigned char *bytes, size_t len)
{
	size_t i, j;
	unsigned char swap;
...
{
	unsigned long i, j, shift_bytes, size_bits;
	unsigned char val, hi_bits, lo_bits;
	int all_vals;
	struct eba_s *tmp;
	...
	hi_bits = positions % EBA_CHAR_BIT;
	if (hi_bits == 0) {
		hi_bits = EBA_CHAR_BIT;
		lo_bits = 0;
	} else {
		lo_bits = EBA_CHAR_BIT - hi_bits;
	}

	Eba_copy_on_stack(eba, tmp, Eba_crash);
	Eba_memset(eba->bits, 0, eba->size_bytes);
	for (i = 0; i < eba->size_bytes; ++i) {
		/* first we want the high bits */
		val = tmp->bits[i] >> lo_bits;
		j = i + shift_bytes;
		if (j >= eba->size_bytes && fill == eba_fill_ring) {
			j = (j - eba->size_bytes);
		}
		if (j < eba->size_bytes) {
			eba->bits[j] |= val;
		...
		/* then we want th low bits */
		if (lo_bits) {
			val = tmp->bits[i] << hi_bits;
			++j;
			if (j >= eba->size_bytes && fill == eba_fill_ring) {
				j = (j - eba->size_bytes);
			}
...
#endif
		reverse_bytes_unsafe(eba->bits, eba->size_bytes);
#if Eba_need_endian
	}
#endif
